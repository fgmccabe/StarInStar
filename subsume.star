subsume is package{
  ...
  subsume has type (dictionary) => (iType,iType)=>result of ();
  subsume(D) is let{
    var resets := list of {};
    var resetLevel := 0;
    ...
    sub(t1,v1,t2,v2) is case (v1,v2) in {
      (iTvar{},iTvar{}) is chainLength(t1)<chainLength(t2) ?
	bindVar(v1,v2) | bindVar(v2,v1);
	...
	f1 is freshenForUse(v1,D);
	valis sub(f1,deRef(f1),t2,v2);
      };
      (_,iUniv(_,_,_)) is valof{
	f2 is freshenForEvidence(v2,D);
    ...
    subList(list of {A1;..T1},list of {A2;..T2}) is valof{
      R is sub(A1,deRef(A1),A2,deRef(A2));
	...
	for K->V1 in M1 do {
	  if M2[K] matches V2 then {
	    R is sub(V1,deRef(V1),V2,deRef(V2));
