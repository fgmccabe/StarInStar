pairHeap is package{
  ...
  fun pairHeap(O) is priorityQ{
    def elem is O;
    private type heapQ is emptyHeap or T(elem.t,list of heapQ);
    type heapQ counts as heap;

    def empty is emptyHeap;
    fun isEmpty(emptyHeap) is true
     |  isEmpty(_) default is false

    fun insrt(x,h) is mrg(T(x,list of []),h)

    fun mrg(h,emptyHeap) is h
    ...
    private
    fun mergePairs(list of []) is emptyHeap
     |  mergePairs(list of [H]) is H
     |  mergePairs(list of [h1,h2,..H]) is mrg(mrg(h1,h2),mergePairs(H))

    fun findMin(emptyHeap) is raise "empty"
     |  findMin(T(x,_)) is x;

    fun deleteMin(emptyHeap) is raise "empty"
