types is package{
  ...
  type iConstraint is iContract(string,list of iType,list of iType) or -- contract constraint The type argument includes all affected types
  ...
  showTypeList(L) is ppSequence(0,
      cons of [
        ppStr("("),
        ppSequence(0,
  ...
  showConstraint(iContract(Ct,At,list of [])) is
    ppSequence(0,cons of [ ppStr(Ct), ppStr(" over "), showTypeList(At)]);
  showConstraint(iContract(Ct,At,Dt)) is
    ppSequence(0,cons of [ ppStr(Ct), ppStr(" over "), showTypeList(At), ppStr(" determines "), showTypeList(Dt)]);
  ...
  varsInType(T) is let{
    ...
    inCon(soFar,iContract(Ct,Tps,Dts)) is leftFold(varsIn,leftFold(varsIn,soFar,Tps),Dts);
  ...
  mapFold(F,I,D) is leftFold(fn(A,(_,T))=>F(A,T),I,D)
